...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case OPEN_CELL: {
      ...
      const checked = []; //한 번 검사한 칸은 다시 검사하지않기위해
      let openedCount = 0;
      ...
      const checkAround = (row, cell) => {
        ...
        if (count === 0) { // 주변칸 오픈
          if (row > -1) { 
            ...
            near.push([row, cell + 1]);
            if (row + 1 < tableData.length) { //현재 제일 마지막 행일 때
              near.push([row + 1, cell - 1]);
              near.push([row + 1, cell]);
              near.push([row + 1, cell + 1]);
            }
      ...
      checkAround(action.row, action.cell); //내 기준으로 빈칸확인하기
      let halted = false;
      let result = '';
